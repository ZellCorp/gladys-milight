        ...
        switch(params.deviceType.type){
            case 'binary':
                if(params.state.value === 1){
                    light.commandsV6.sendCommands(commands.bridge.on());
                } else {
                    light.commandsV6.sendCommands(commands.bridge.off());
                }
            break;
            case 'brightness':
                light.commandsV6.sendCommands(commands.bridge.on());
                light.commandsV6.sendCommands(commands.bridge.brightness(params.state.value));
            break;

            case 'hue':
                light.commandsV6.sendCommands(commands.bridge.on());
                light.commandsV6.sendCommands(commands.bridge.hue(params.state.value));
            break;
        }
    } else {
        switch(params.deviceType.type){
